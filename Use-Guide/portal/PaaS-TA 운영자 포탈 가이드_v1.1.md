...
2. 조직이름 옆의 ①상세정보 버튼을 눌러서 ②조직 상세 정보를 조회한다.<br>
![07]

3. 공간이름 옆의 ①상세정보 버튼을 눌러서 ②공간 영역 상세 정보를 조회한다.<br>
![08]
...
2. **클라이언트 ID**를 클릭하면 클라이언트 상세화면을 이동한다. 클라이언트 상세화면에서 클라이언트 ID 삭제가 가능하다.<br>
![10]

3. ①"클라이언트 등록" 버튼을 클릭하여 클라이언트 아이디, 패스워드, 옵션, 옵션 가이드를 입력하고 ②"등록" 버튼을 클릭하여 등록한다.<br>
![11]
...
3. 앱 템플릿을 삭제하기 위해 하단의 ②"삭제" 버튼을 클릭한다.
![15]

4. 카탈로그 화면에서 앱 템플릿을 등록하기 위해 ①"앱 템플릿 등록" 버튼을 클릭하면 다음의 화면이 나온다. 이름, 분류, 앱 개발환경, 서비스, 썸네일, 공개여부, 요약, 설명을 입력하고 ②"등록" 버튼을 누르면 새로운 앱 템플릿이 등록된다.
![16]
...
2. 앱 개발환경 이름을 클릭하면 상세화면이 나온다. 수정할 내역이 있을 경우 수정 한 후 ①"저장"버튼을 클릭하여 저장한다.
![18]

3. 앱 개발환경을 삭제하기 위해 하단의 ②"삭제" 버튼을 클릭한다.
![19]

4. 카탈로그 화면에서 앱 개발환경을 등록하기 위해 ①"앱 개발환경 등록" 버튼을 클릭하면 다음의 화면이 나온다. 이름, 분류, 앱 개발환경, 서비스, 썸네일, 공개여부, 요약, 설명을 입력하고 ②"등록" 버튼을 누르면 새로운 앱 개발환경이 등록된다.
![20]

### <a name="4.3.3"/>  4.3.3 카탈로그 관리_서비스
1. 카탈로그 서비스 목록을 조회한다. 이름, 요약, 분류, 공개여부를 조회할 수 있다. 검색창에서 검색유형, 검색항목, 공개여부, 검색어로 검색이 가능하다.<br>
![21]

2. 서비스 이름을 클릭하면 상세화면이 나온다. 수정할 내역이 있을 경우 수정 한 후 ①"저장"버튼을 클릭하여 저장한다.<br>
![22]

3. 서비스를 삭제하기 위해 하단의 ②"삭제" 버튼을 클릭한다.<br>
![23]

4. 카탈로그 화면에서 앱 서비스를 등록하기 위해 ①"앱 서비스 등록버튼"을 누르면 다음의 화면이 나온다. 
이름, 분류, 서비스, 썸네일, 파라미터, 공개여부, 요약, 설명을 입력하고 ②"등록" 버튼을 누르면 새로운 서비스가 등록된다. <br>
![24]
...
6. 문서 URL을 넣어준다. 
     ...
     > **문서 URL 적용 시 'PaaS-TA 사용자'카탈로그 서비스 등록시 '문서보기' 버튼 활성화 된다.**
...
6. 문서 URL을 넣어준다.
     ...
     > **문서 URL 적용 시 'PaaS-TA 사용자'카탈로그 서비스 등록시 '문서보기' 버튼 활성화 된다.**<br>
7. 서비스 생성시 필요한 파라미터값을 설정한다.
    (예) owner : '자동입력' /  org_name : '자동입력'
8. 앱 바인드 사용 은 "N"으로 선택한다.
9. 대시보드 사용 시 "Y" 를 필수 설정한다. 대시보드 "N" 설정시 대시보드 버튼이 비 활성화 된다.
...
2. ①"관리" 버튼 클릭 후 "비밀번호 초기화"를 선택해 비밀번호를 초기화 한다.<br>
![26]

3. ①"관리" 버튼 클릭 후 "운영자권한 삭제"를 선택해 운영권한 삭제를 한다.<br>
![28]
![27]
     > "실행" 버튼 클릭 후 사용자 관리 리스트를 확인하면 [관리자] **Y**에서 **N** 으로 변한다. 사용자 관리 리스트에서 **N** 경우 ①"관리" 버튼 클릭 후 "운영자권한 부여"를 클릭한다. <br>
     > ![27-1]
4. ①"관리" 버튼 클릭 후 "가입 해제"를 선택해 가입을 해제한다.<br>
![30]
![31]
     > "수정" 버튼 클릭 후 사용자 관리 리스트를 확인하면 [상태] **승인**에서 **비승인**으로 변한다. <br>
     > ![31-1]
5. ①"관리" 버튼 클릭 후 "계정 삭제"를 선택해 해당 계정을 삭제한다.<br>
![32]
![33]
...
2. ①"그룹 생성" 버튼을 클릭 후 그룹 이름을 입력한다. ②"저장" 버튼 클릭하여 권한그룹을 생성한다.<br>   
![35]

3. ① 권한 그룹에서 삭제할 그룹을 선택한다. 확인 폼 레이어가 팝업되면 ②"삭제"를 클릭하여 권한 그룹을 삭제한다.<br>
![36]

4. ① 권한 그룹 리스트에서 해당 그룹을 클릭하여 사용자를 확인한다. ②"유저 등록" 버튼을 클릭한다. ③ 리스트에서 사용자를 클릭하고 ④"저장"버튼을 클릭하여 유저를 등록한다.<br>   
![37]

5. ① 권한 그룹 리스트에서 해당 그룹을 클릭하여 사용자를 확인한다. 사용자 리스트에서 삭제할 사용자 ②체크박스 선택한다. ③ 활성화 된 "유저 삭제" 버튼을 클릭한다. 팝업의 ④ "삭제" 를 클릭하여 유저를 삭제한다.<br>
![38]
...
2. 조직 할당량 등록을 하기 위해 이름, 메모리, 인스턴스 메모리, 라우트, 서비스 인스턴스, APP 인스턴스, 무료여부, 예약된 라우트 포트를 입력하고 "등록" 버튼을 클릭한다.<br>
![48]

3. 수정할 조직 할당량을 클릭하고 조직 할당량 상세 팝업에서 ① "저장" 버튼을 클릭하여 조직 할당량을 저장한다.<br>
![49]

4. 삭제할 조직 할당량을 클릭하고 조직 할당량 상세 팝업에서 ② "삭제" 버튼을 클릭하여 조직 할당량을 삭제한다.<br>
![49]

5. "공간 할당량 등록" 버튼을 클릭하여 공 할당량을 등록한다.<br>
![58]

6. 공간 할당량 등록을 하기 위해 이름, 생성조직명, 메모리, 인스턴스 메모리, 라우트, 서비스 인스턴스, APP 인스턴스, 무료여부, 예약된 라우트 포트를 입력하고 "등록" 버튼을 클릭한다.<br>
     ![59]

7. 삭제할 공간 할당량을 클릭하고 "삭제" 버튼을 클릭하여 공간 할당량을 삭제한다.<br>
...
4.  서비스 브로커 리스트의 ①"서비스 브로커 등록" 버튼을 클릭한다. 서비스 브로커 이름, 사용자 이름, 사용자 패스워드,URL 정보를 입력하고 ②"등록"버튼을 클릭하면 서비스 브로커 등록이 완료된다.<br>
![57]
